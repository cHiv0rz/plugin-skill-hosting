...
// before any auth round-trip. For signed-in users the auth store reconciles
// this with the server-side preference (which wins across devices).
//
// NOTE: index.html contains a tiny inline pre-boot copy of this logic to avoid
// a flash-of-wrong-theme before the bundle loads. If THEME_IDS changes here,
// update the list in index.html too.
...
// default — so a stale/garbage localStorage value or an unexpected server
// response can never leave the UI in an unstyled state.
